...
import { StripeMinuteProvider, configuredStripeMinuteProvider, type StripeMinuteConfig, type StripeMinuteTransport } from '../src/stripe-minute-provider.js';
...
async function fixture() {
  ...
  const stripe = new StripeMinuteProvider(db, vault, stripeConfig(), stripeTransport), play = new PlayMinuteProvider(db, vault, playConfig(), playTransport);
  ...
  return { db, schema, encryptionKey, vault, stripeTransport, playTransport, stripe, play, catalog, purchases, worker,
    ...
    async stripeOrder(account: string) { const order = await purchases.createOrder(account, 'stripe', 'synthetic-thirty', randomUUID()); stripeTransport.bind(order.orderID); return order; },
...
integration('Stripe wrong merchant, changed price and old uncertain checkout attempts cannot open a new payment', async () => {
  ...
  try {
    ...
    await assert.rejects(f.stripe.checkout(account, order.orderID), /checkout_reconciliation_required/);
...
integration('restricted runtime can store encrypted receipts and process jobs but cannot rewrite bindings', async () => {
  const f = await fixture(), role = `delivery_role_${randomUUID().replaceAll('-', '')}`;
