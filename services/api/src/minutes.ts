...
export async function lockMinuteWallet(sql: PoolClient, account: string, active = true) {
  ...
  const row = (await sql.query('SELECT balance_ms,reserved_ms FROM minute_wallets WHERE account_id=$1 FOR UPDATE', [account])).rows[0];
  return { balance: Number(row.balance_ms), reserved: Number(row.reserved_ms) };
}
...
export async function appendMinuteEntry(sql: PoolClient, account: string, reference: string, kind: EntryKind,
  balanceDelta: number, reservedDelta: number): Promise<boolean> {
  ...
  if (existing) {
    if (existing.account_id !== account || existing.kind !== kind || Number(existing.balance_delta_ms) !== balanceDelta ||
      Number(existing.reserved_delta_ms) !== reservedDelta) throw new ServiceError('idempotency_conflict', 409);
    return false;
  }
  ...
  await sql.query(`INSERT INTO minute_entries(id,account_id,reference,kind,balance_delta_ms,reserved_delta_ms)
    VALUES($1,$2,$3,$4,$5,$6)`, [randomUUID(), account, reference, kind, balanceDelta, reservedDelta]);
  await sql.query('UPDATE minute_wallets SET balance_ms=$2,reserved_ms=$3 WHERE account_id=$1', [account, balance, reserved]);
  return true;
}

export async function minuteBalance(db: Database, account: string) {
  ...
  return { unit: 'milliseconds' as const, balanceMilliseconds: Number(row.balance_ms),
    reservedMilliseconds: Number(row.reserved_ms), availableMilliseconds: Number(row.balance_ms) - Number(row.reserved_ms),
...
export async function assertWelcomeBudget(sql: PoolClient, policy: any, allowance: number) {
  const usage = (await sql.query(`SELECT COALESCE(sum(balance_delta_ms),0) AS lifetime,
    COALESCE(sum(balance_delta_ms) FILTER (WHERE created_at>=date_trunc('day',now() AT TIME ZONE 'UTC') AT TIME ZONE 'UTC'),0) AS daily
    FROM minute_entries WHERE kind='welcome'`)).rows[0];
  if (BigInt(usage.lifetime) + BigInt(allowance) > BigInt(policy.lifetime_welcome_budget_ms) ||
    BigInt(usage.daily) + BigInt(allowance) > BigInt(policy.daily_welcome_budget_ms)) throw new ServiceError('welcome_budget_reached', 503);
}
