...
Google's External/Testing state normally uses a test-user allowlist, but basic identity scopes (`openid`, `email`, `profile`) are an exception: other Google users can authorize too. Mural requests basic identity scopes, so Testing is not an invitation-only signup control. Workspace policies can still block authorization. [Google OAuth app states](https://developers.google.com/identity/protocols/oauth2/production-readiness/overview)
