...
Admission limits are transactional and shared by every API instance: five valid submissions per network address per UTC hour, 200 total per hour, 1,000 total per UTC day, 500 new email addresses per day, and 10,000 retained addresses. Duplicates and honeypots consume request limits. Invalid bodies are rejected before persistence. Once storage or daily signup capacity is full, both new and existing addresses receive the same unavailable response. These are initial operating limits, not protection against a large denial-of-service attack.
